...
public final class PowerTurbineTest {
    ...
    public static void test01_massAndEnergyAcrossBothStages(){
        ...
        var hp=PowerTurbine.expand(PowerTurbine.Stage.HP,main,cross,100,1e9,0);
        near(hp.mass(),100);near(hp.workKJ(),28000);near(cross.energyKJ()+hp.workKJ(),277000);
        var lp=PowerTurbine.expand(PowerTurbine.Stage.LP,cross,null,100,1e9,100);
    ...
    public static void test02_bufferAndLoadLimitsNeverConsumeUnaccountedSteam(){
        ...
        var hp=PowerTurbine.expand(PowerTurbine.Stage.HP,main,cross,100,1400,0);near(hp.mass(),5);near(main.mass(),95);
        hp=PowerTurbine.expand(PowerTurbine.Stage.HP,main,cross,100,1e9,0);near(hp.mass(),5);near(main.mass(),90);
        hp=PowerTurbine.expand(PowerTurbine.Stage.HP,main,cross,100,1e9,0);near(hp.mass(),0);near(main.mass(),90);
        var lp=PowerTurbine.expand(PowerTurbine.Stage.LP,cross,null,100,1e9,2.25);near(lp.mass(),2.25);near(cross.mass(),7.75);
        lp=PowerTurbine.expand(PowerTurbine.Stage.LP,cross,null,100,0,200);near(lp.mass(),0);near(cross.mass(),7.75);
    }
    public static void test03_parallelDrawsConserveHeaderInventory(){
        ...
        double mass=0,work=0,heat=0;
        for(int i=0;i<4;i++){var lp=PowerTurbine.expand(PowerTurbine.Stage.LP,header,null,20,1e9,200);mass+=lp.mass();work+=lp.workKJ();heat+=lp.rejectedHeatKJ();}
        near(mass,40.75);near(header.mass(),0);near(work+heat+mass*167.2,40.75*2490);
    }
