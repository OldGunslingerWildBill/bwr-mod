...
def seal_shell_transition(s):
    ...
    for o in s.objects:
        ...
        for i in (0,1,4,5):
            ...
            v.co=xyz((low_x+skin if x<3.5 else high_x-skin,
                      join_y,low_z+skin if z<3.5 else high_z-skin))
        o.data.update()
...
def build_ports():
    """One bay per 7x5x7 LP module, with a fitted seating collar and full-size ports."""
    source=bpy.data.scenes.get('BWR | Arabelle 1700 condenser exterior')
    if source is None:
        with bpy.data.libraries.load(str(ROOT/'art/models/condenser_msiv/arabelle_1700_condenser.blend')) as (src,dst):
            dst.scenes=['BWR | Arabelle 1700 condenser exterior']
    ...
    s=new_scene('BWR | Compact LP condenser')
    skip=('titanium tubes','perforated tube sheets','tube support','bundle tie','shell stay',
          'neck feedwater heater','heater head','dished heater cover','heater instrument elbow',
    ...
    for o in source.objects:
        ...
        for vert in clone.data.vertices:
            ...
            vert.co=xyz((x*.875,compress_y(y),1.18+(z-2.15)*4.64/18.16))
        clone.data.update()
    box(s,'center foundation crossbeam',(.5,.18,3.1),(6.5,.43,3.9),CONCRETE)
    # Shaped transition matches the LP lower exhaust casing; seating rim matches its skid.
    lo=(.73,3.75,1.35,6.27,4.65,5.65)
    ...
    for x in (.24,6.76):
        for z in (1,2,3,4,5,6):bolt(s,'LP seating stud',(x,5.87,z),(x,5.99,z),.04)
    ports=[]
    ...
    for hot in (True,False):
        role='HOT' if hot else 'COLD';sign=-1 if hot else 1;zc=1.05 if hot else 5.95;end=.5 if hot else 6.5
        for x in (1.5,5.5):
            name=role+' cooling water '+str(x)
            path=[(x,2.5,1.30 if hot else 5.70)]+[(x,1.95+.55*math.cos(i*math.pi/32),zc+sign*.55*math.sin(i*math.pi/32)) for i in range(17)]+[(x,1.35,end)]
    ...
    sleeve(s,'condensate outlet neck',(3.5,1.5,5.4),(3.5,1.5,6.88),.25,.205,STEEL,24)
    steam_flange(s,'condensate outlet flange',(3.5,1.5,7),(0,0,1))
    endpoint('CONDENSATE',[3,1,6],'south')
    ...
    for x in (.38,2,5,6.62):cylinder(s,'guardrail upright',(x,3.5,.77),(x,4.07,.77),.025,STEEL,8)
    ...
    meta={'id':'arabelle_condenser','size':[7,6,7],'controller':[3,0,3],'ports':ports,'layout_version':2,
          'lp_root_offset':[0,6,0],'reference_plant_mwe':1700,'reference_heat_rejection_mw':2750,'cutaway':False}
    s['layout']=json.dumps(meta);s['description']='One compact closed condenser per LP module; actual LP skid/casing define the top collar.'
    export_scene(s,'arabelle_condenser.json',meta)
    review=new_scene('BWR | Compact condenser studio')
    for o in s.objects:review.collection.objects.link(o)
    studio(review,(3.5,2.8,3.5),(14,10,-13),11,(1400,1100))
    fitted=new_scene('BWR | LP and fitted condenser')
    for o in s.objects:fitted.collection.objects.link(o)
    for o in lp.objects:
        if o.type!='MESH':continue
        clone=o.copy();fitted.collection.objects.link(clone);clone.location+=xyz((0,6,0))
    studio(fitted,(3.5,5.2,3.5),(16,13,-16),16,(1200,1400))
    viewport(fitted,(3.5,5.2,3.5),(16,13,-16),20)
    bpy.data.libraries.write(str(OUT/'condenser_ports.blend'),{s,review,fitted},fake_user=True)
    print('Compact condenser:',len(ports),'ports; LP root exactly 6 blocks above condenser root; source bounds',skid,casing)
