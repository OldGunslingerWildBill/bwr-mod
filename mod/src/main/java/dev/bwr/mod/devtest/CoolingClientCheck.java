...
public final class CoolingClientCheck {
    ...
    private static final String[] NAMES={"natural-draft","circular-induced-draft","water-pumps-intake","fan-controls","intake-controls","submerged-intake"};
    private static final double[][] CAMERAS={{175,224,94,35,10},{194,216,112,38,18},{214,208,124,35,14},{173,202,130,0,0},{211,201,137,0,0},{213,201.3,137.3,37,5}};
    ...
    @SubscribeEvent public static void supply(net.neoforged.neoforge.event.tick.ServerTickEvent.Post event){
        ...
        for(var root:ROOTS)if(l.getBlockEntity(root) instanceof CoolingBlockEntity be&&be.design().watts>0){be.power().receiveEnergy(Integer.MAX_VALUE,false);if(be.design().tower){be.plant().fillInput(500,false);be.plant().drain(500,false);}}
    }
    @SubscribeEvent public static void tick(net.neoforged.neoforge.client.event.ClientTickEvent.Post e){
        ...
        if(++shown<90)return;mc.options.hideGui=stage<3||stage==5;mc.getToasts().clear();if(shown<95)return;
