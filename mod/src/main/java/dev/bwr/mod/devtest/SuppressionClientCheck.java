...
public final class SuppressionClientCheck {
    ...
    private static final String[] NAMES={"concrete-basin","exchanger-ports","exchanger-panel","pool-panel"};
    private static boolean started,requested,fixture,warming;private static int age,joined,stage,shown,warmTicks;
    ...
    @SubscribeEvent public static void tick(net.neoforged.neoforge.client.event.ClientTickEvent.Post e) {
        ...
        var camera=stage==0?new Vec3(181,204,141):stage==1?new Vec3(184,195,162):stage==2?new Vec3(166.5,197,154):new Vec3(166.5,192,158);
        var target=stage==0?new Vec3(167,192.5,162):stage==1?new Vec3(183.5,192,168):stage==2?Vec3.atCenterOf(HX):Vec3.atCenterOf(O.offset(6,1,0));
        ...
        if(stage==3&&(!(mc.player.containerMenu instanceof SuppressionPoolMenu m)||!m.formed||!m.concrete||m.physicalCoolingMW<=0))throw new AssertionError("Wall-port pool panel failed to open/sync");
        ...
        mc.player.setPos(camera.x,camera.y,camera.z);mc.player.setYRot(yaw);mc.player.setXRot(pitch);mc.player.setDeltaMovement(Vec3.ZERO);mc.options.hideGui=stage<2;mc.getToasts().clear();if(shown<110)return;
        ...
        if(++stage==NAMES.length){com.mojang.logging.LogUtils.getLogger().info("SUPPRESSION CLIENT CHECK PASS: concrete basin, four flange orientations, pipe seams, live exchanger and wall-port panels");mc.stop();}
    }
}
