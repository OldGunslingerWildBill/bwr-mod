...
public final class CoolingLayout {
    private static final Map<dev.bwr.core.turbine.CoolingWaterUnit.Design,CoolingLayout> CACHE=new EnumMap<>(dev.bwr.core.turbine.CoolingWaterUnit.Design.class);
    ...
    public final List<net.minecraft.world.phys.Vec3> rotors=new ArrayList<>();
    private final Map<Integer,Cell> byIndex;
    private CoolingLayout(String resource){
        try(var stream=getClass().getResourceAsStream("/data/bwr/cooling/"+resource)){
            if(stream==null)throw new IllegalStateException("Missing cooling layout: "+resource);
            var json=JsonParser.parseReader(new InputStreamReader(stream,StandardCharsets.UTF_8)).getAsJsonObject();
            size=pos(json.getAsJsonArray("size"));controller=pos(json.getAsJsonArray("controller"));
            for(var r:json.getAsJsonArray("rotors")){var a=r.getAsJsonArray();rotors.add(new net.minecraft.world.phys.Vec3(a.get(0).getAsDouble(),a.get(1).getAsDouble(),a.get(2).getAsDouble()));}
            ...
            for(var e:json.getAsJsonArray("cells")){
                ...
                int index=c.get("index").getAsInt();entries.put(index,new Cell(index,pos(c.getAsJsonArray("cell")),CoolingBlock.Port.valueOf(c.get("role").getAsString()),Direction.byName(c.get("face").getAsString()),shapes));
            }
            byIndex=Map.copyOf(entries);cells=List.copyOf(entries.values());ports=cells.stream().filter(c->c.role()!=CoolingBlock.Port.NONE).toList();
        }catch(java.io.IOException e){throw new IllegalStateException("Cannot load cooling layout: "+resource,e);}
    }
