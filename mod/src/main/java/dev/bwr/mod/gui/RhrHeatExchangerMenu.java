...
public class RhrHeatExchangerMenu extends BwrMenu {
    ...
    public double cold,hot,primaryFlow,secondaryFlow,heat,hotTemperature,primaryOut,coldTemperature;
    ...
    @Override protected void writeSnapshot(FriendlyByteBuf b){var be=blockEntity(RhrHeatExchangerBlockEntity.class);b.writeBoolean(be!=null);if(be==null)return;var h=be.plant();b.writeBoolean(be.returnPool()!=null);for(double n:new double[]{h.cold(),h.hot(),h.primaryFlow(),h.secondaryFlow(),h.heatMW(),h.secondaryOutC(),h.primaryOutC(),h.coldTemperatureC()})b.writeDouble(n);}
    @Override protected void readSnapshot(FriendlyByteBuf b){present=b.readBoolean();if(!present)return;connected=b.readBoolean();cold=b.readDouble();hot=b.readDouble();primaryFlow=b.readDouble();secondaryFlow=b.readDouble();heat=b.readDouble();hotTemperature=b.readDouble();primaryOut=b.readDouble();coldTemperature=b.readDouble();}
