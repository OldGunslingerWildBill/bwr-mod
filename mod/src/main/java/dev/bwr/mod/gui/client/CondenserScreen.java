...
public class CondenserScreen extends BwrScreen<CondenserMenu> {
    public CondenserScreen(CondenserMenu menu,Inventory inv,Component title){super(menu,inv,title,null,310,224);}
    ...
    @Override protected void renderContent(GuiGraphics g,int x,int y){
        text(g,menu.present&&menu.ready?"Steam bypass condenser":"Waiting for complete, loaded condenser",12,32,TEXT);
        ...
        readout(g,"Condensate",big(menu.condensate)+" kg",12,87,298,TEXT_BRIGHT);
        ...
        text(g,"Assumed cooling: 13 -> 24 C; condensate: 40 C",12,180,TEXT_DIM);
        text(g,"Supply cold water and drain both outputs.",12,194,TEXT_DIM);
        text(g,"Return hot water to a cooling tower.",12,208,TEXT_DIM);
    }
}
